...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('v_antieveryone')
    .setDescription('実行したチャンネルでの@everyone連投自動削除を設定します')
  ...
  async execute(interaction, client) {
    ...
    config.channels[interaction.channelId].antiEveryone = status;
    await client.saveSettings(allSettings);

    return interaction.reply({ 
      content: `✅ <#${interaction.channelId}> での **@everyone 自動削除を ${status ? '🟢 ON' : '🔴 OFF'} に設定しました。**`, 
