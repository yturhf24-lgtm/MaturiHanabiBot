...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('evacuation-info')
    .setDescription('【特別ユーザー・サーバーオーナー専用】自治体の避難情報（避難勧告等）を設定します')
  ...
  async execute(interaction, client) {
    ...
    settings[guildId].evacuationInfo = {
      enabled: status === 'on',
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setTitle(` evacuation-info: ${status.toUpperCase()}`)
          .setTitle(`🚨 避難情報通知: ${status.toUpperCase()}`)
