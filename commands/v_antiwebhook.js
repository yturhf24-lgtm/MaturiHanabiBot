...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('実行したチャンネルでのWebhookスパム自動削除を設定します')
  ...
  async execute(interaction, client) {
    ...
    config.channels[interaction.channelId].antiWebhook = status;
    await client.saveSettings(allSettings);

    return interaction.reply({ 
      content: `✅ <#${interaction.channelId}> での **Webhookスパム対策を ${status ? '🟢 ON' : '🔴 OFF'} に設定しました。**`, 
