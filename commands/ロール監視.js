...
module.exports = {
  ...
  async handleSelect(i, step) {
    ...
    if (i.user.id !== userId) {
      if (!i.replied && !i.deferred) await i.reply({ content: '⛔ 本人専用です', flags: 64 });
      return;
    }
    ...
    // === 戻るボタン ===
    if (isBack) {
      const s = step.replace('-back','');
      if (s === 'step2' || s === 'step3' || s === 'step4') {
    ...
    if (step === 'step4') {
      ...
      if (has) {
        ...
        if (!rm.length && !add.length) res += '変更はありません';

        logtxt = `📋 ロール監視 実行ログ\n👤 ${mem.user.tag}\n✅ 確認：<@&${data.checkRoleId}> → 保持中\n`;
        if (rm.length) logtxt += `🗑️ 削除：${rm.join(', ')}\n`;
        if (add.length) logtxt += `🎁 付与：${add.join(', ')}\n`;
        logtxt += `🕒 ${new Date().toLocaleString('ja-JP')}`;
      } else {
      ...
      try {
        ...
        if (ch?.send) await ch.send({ content: logtxt });
      } catch (e) {}
